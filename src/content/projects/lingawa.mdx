...
<MediaFull
  width="content"
  src="/lingawa/user-flow-ia.svg"
  alt="How learners move through Lingawa. Onboarding: language selection, then a proficiency assessment that determines starting level, then an age check that routes to either the adult or kids path. Adult path: Home leads to Live Lessons, Practice, and Progress Tracker; Practice opens the seven modalities (lesson slides, vocab drills, matching game, dialogue practice, sentence construction, story-time reader, practice hub), each staged Beginner to Advanced, while Progress Tracker aggregates every modality and syncs back to Home. Kids path: a learning map leading to Games, Stories, and Songs, using the same design system with a different IA and larger touch targets. One system, two architectures, 70% component reuse."
/>
